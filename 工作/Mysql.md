...
- **Serializable** (串行化)：通过强制事务[排序](https://www.nowcoder.com/jump/super-jump/word?word=排序)，使之不可能相互冲突，从而解决幻读问题。
...
3. 经常需要[排序](https://www.nowcoder.com/jump/super-jump/word?word=排序)的字段建立索引，因为索引已经排好序，可以加快[排序](https://www.nowcoder.com/jump/super-jump/word?word=排序)查询速度
...
索引的数据结构主要有B+树和[哈希表](https://www.nowcoder.com/jump/super-jump/word?word=哈希表)，对应的索引分别为B+树索引和哈希索引。InnoDB引擎的索引类型有B+树索引和哈希索引，默认的索引类型为B+树索引。
...
![img](assets/Mysql/Bplus%E6%A0%91%E7%B4%A2%E5%BC%95.png)

进行查找操作时，首先在根节点进行[二分查找](https://www.nowcoder.com/jump/super-jump/word?word=二分查找)，找到`key`所在的指针，然后递归地在指针所指向的节点进行查找。直到查找到叶子节点，然后在叶子节点上进行[二分查找](https://www.nowcoder.com/jump/super-jump/word?word=二分查找)，找出`key`所对应的数据项。
...
哈希索引是基于[哈希表](https://www.nowcoder.com/jump/super-jump/word?word=哈希表)实现的，对于每一行数据，存储引擎会对索引列进行哈希计算得到哈希码，并且哈希[算法](https://www.nowcoder.com/jump/super-jump/word?word=算法)要尽量保证不同的列值计算出的哈希码值是不同的，将哈希码的值作为[哈希表](https://www.nowcoder.com/jump/super-jump/word?word=哈希表)的key值，将指向数据行的指针作为[哈希表](https://www.nowcoder.com/jump/super-jump/word?word=哈希表)的value值。这样查找一个数据的时间复杂度就是O(1)，一般多用于精确查找。
...
- 哈希索引**不支持[排序](https://www.nowcoder.com/jump/super-jump/word?word=排序)**，因为[哈希表](https://www.nowcoder.com/jump/super-jump/word?word=哈希表)是无序的。
- 哈希索引**不支持范围查找**。
- 哈希索引**不支持模糊查询**及多列索引的最左前缀匹配。
- 因为[哈希表](https://www.nowcoder.com/jump/super-jump/word?word=哈希表)中会**存在哈希冲突**，所以哈希索引的性能是不稳定的，而B+树索引的性能是相对稳定的，每次查询都是从根节点到叶子节点。
...
如下图，对(a, b) 建立索引，a 在索引树中是全局有序的，而 b 是全局无序，局部有序（当a相等时，会根据b进行[排序](https://www.nowcoder.com/jump/super-jump/word?word=排序)）。直接执行`b = 2`这种查询条件无法使用索引。

![最左前缀](assets/Mysql/%E6%9C%80%E5%B7%A6%E5%89%8D%E7%BC%80.png)
...
InnoDB使用表的主键构造主键索引树，同时叶子节点中存放的即为整张表的记录数据。聚集索引叶子节点的存储是逻辑上连续的，使用双向[链表](https://www.nowcoder.com/jump/super-jump/word?word=链表)连接，叶子节点按照主键的顺序[排序](https://www.nowcoder.com/jump/super-jump/word?word=排序)，因此对于主键的[排序](https://www.nowcoder.com/jump/super-jump/word?word=排序)查找和范围查找速度比较快。
...
![img](assets/Mysql/cover-index.png)
...
1. 哈希索引数据不是按照索引值顺序存储，无法用于[排序](https://www.nowcoder.com/jump/super-jump/word?word=排序)。
2. 不支持部分索引匹配查找，因为哈希索引是使用索引列的全部内容来计算哈希值的。
3. 只支持等值比较，不支持范围查询。
4. 当出现哈希冲突时，存储引擎需要遍历[链表](https://www.nowcoder.com/jump/super-jump/word?word=链表)中所有的行指针，逐行进行比较，直到找到符合条件的行。
...
![img](assets/Mysql/mvcc2.png)

2、事务A对该行数据做了修改，将`age`修改为12，效果如下：

![img](assets/Mysql/mvcc7.png)

3、之后事务B也对该行记录做了修改，将`age`修改为8，效果如下：

![img](assets/Mysql/mvcc11.png)
...
在`read view`内部维护一个活跃事务[链表](https://www.nowcoder.com/jump/super-jump/word?word=链表)，表示生成`read view`的时候还在活跃的事务。这个[链表](https://www.nowcoder.com/jump/super-jump/word?word=链表)包含在创建`read view`之前还未提交的事务，不包含创建`read view`之后提交的事务。
...
![img](assets/Mysql/read_view10.png)
...
- 如果
  ...
  1. 需要在活跃事务[链表](https://www.nowcoder.com/jump/super-jump/word?word=链表)中查找是否存在ID为`DATA_TRX_ID`的值的事务。
  2. 如果存在，因为在活跃事务[链表](https://www.nowcoder.com/jump/super-jump/word?word=链表)中的事务是未提交的，所以该记录是不可见的。此时需要通过版本链找到上一个版本，然后重新判断该版本的可见性。
...
![img](assets/Mysql/image-20210922232259664.png)
...
![img](assets/Mysql/%E5%B9%BB%E8%AF%BB1.png)
...
![img](assets/Mysql/%E5%9E%82%E7%9B%B4%E5%88%92%E5%88%86.png)
...
![img](assets/Mysql/%E6%B0%B4%E5%B9%B3%E5%88%92%E5%88%86.png)
...
4. 两种执行方案，先查 `id > 1` 还是 `name = '大彬'`，优化器根据自己的优化[算法](https://www.nowcoder.com/jump/super-jump/word?word=算法)选择执行效率最好的方案；
...
- 乐观锁：假设不会发生并发冲突，只在提交操作时检查是否数据是否被修改过。给表增加`version`字段，在修改提交之前检查`version`与原来取到的`version`值是否相等，若相等，表示数据没有被修改，可以更新，否则，数据为脏数据，不能更新。实现方式：乐观锁一般使用版本号机制或`CAS`[算法](https://www.nowcoder.com/jump/super-jump/word?word=算法)实现。
...
7. state
   ...
   - Connect，一个从节点连上了主节点
   - Quit，线程正在退出
   - Sorting for group，正在为`GROUP BY`做[排序](https://www.nowcoder.com/jump/super-jump/word?word=排序)
   - Sorting for order，正在为`ORDER BY`做[排序](https://www.nowcoder.com/jump/super-jump/word?word=排序)
